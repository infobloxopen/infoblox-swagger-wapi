...
<html>
<head><title>Infoblox WAPI Swagger Documentation</title>
...
<div>
  ...
  ?>
    <fieldset>
      <h3>Welcome!! <br/><br/><br/>Please wait while we generate the Swagger documentation!!<br/><br/>
      <?php
      $message = shell_exec("./script.sh ".$_POST["version"]." ".$_POST["host"]);
      ?><br/><hr/><br/>Please click here to view the definition<br/><br/>
  ...
  elseif($_POST["host"] && $_POST["username"] && $_POST["password"]){
    ...
    curl_setopt_array($curl, array(
      CURLOPT_URL => "https://".$_POST["host"]."/wapi/v2.1/?_schema",
    ...
    if(!$result["supported_versions"]){
      ...
      ?>
      <fieldset>
          <h2>Error: Invalid input.</h2><br/><h3> Either this IP-address/FQDN does not correspond to a grid master or the credentials are invalid!!</h3>
...
<fieldset>
    <form  action="home.php" method="post">
            ...
            <select name="version">
              <?php if($calc["2.8"] == 1) echo "<option value='v2.8'>v2.8</option>";$check=1; ?>
              <?php if($calc["2.9"] == 1) echo "<option value='v2.9'>v2.9</option>";$check=1; ?>
              <?php if($calc["2.10"]==1 && $calc["2.11"]!=1) echo "<option value='v2.10' selected>v2.10</option>";$check=1; ?>
              <?php if($calc["2.10"]==1 && $calc["2.11"]==1) echo "<option value='v2.10'>v2.10</option>";$check=1; ?>
              <?php if($calc["2.11"]==1 && $calc["2.12"]!=1) echo "<option value='v2.11' selected>v2.11</option>";$check=1; ?>
              <?php if($calc["2.11"]==1 && $calc["2.12"]==1) echo "<option value='v2.11'>v2.11</option>";$check=1; ?>
              <?php if($calc["2.11.1"] == 1) echo "<option value='v2.11.1'>v2.11.1</option>";$check=1; ?>
              <?php if($calc["2.12"] == 1) echo "<option value='v2.12' selected>v2.12</option>";$check=1; ?>
            </select>
